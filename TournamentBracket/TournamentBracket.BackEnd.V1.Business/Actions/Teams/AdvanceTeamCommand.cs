...
namespace TournamentBracket.BackEnd.V1.Business.Actions.Teams
{
    ...
    public class AdvanceTeamCommandHandler : BackEndGenericHandler, IRequestHandler<AdvanceTeamCommand, AdvanceTeamCommandResult>
    {
        ...
        public async Task<AdvanceTeamCommandResult> Handle(AdvanceTeamCommand request, CancellationToken cancellationToken)
        {
            ...
            var winningTeamsSeedList = new List<string>();


            var roundOf16Winners = new HashSet<string>(request.AdvanceTeamRequest.Events);
            winningTeamsTeamIDs.AddRange(teams.Where(team => roundOf16Winners.Contains(team.Name)).Select(team => team.TeamID));

            foreach (var team in winningTeamsTeamIDs)
            {
                winningTeamsSeedList.Add(teamIDSeedMap[team]);
            }

            var matches = await matchRepository.GetAllMatches(winningTeamsTeamIDs);
            var tournamentID = matches.Select(m => m.TournamentID).FirstOrDefault();
            ...
            await CreateQuaterFinalsMatchFixtures(tournamentID, winningTeamsTeamIDs, winningTeamsSeedList, teamIDSeedMap);
        ...
        private async Task CreateQuaterFinalsMatchFixtures(Guid tournamentID, List<Guid> TeamIDs, List<string> Seeds, Dictionary<Guid, string> teamIDSeedMap)
        {
            ...
            for (int i = 0; i <= 7; i++)
            {
                ...
                if (i < 1)
                {
                    ...
                    i = i + 3;   //to skip other team validation
